...
I found similar values for the two shared parameters with mlp architectures. Medium learning rate, medium batch size. The regularization constant did differ between architecture complexities for the CNNs. I think this can be accounted for by their differing expressive capability. The enhanced cnn will be more likely to over-fit, because of its deeper architecture, and the simple cnn more general because of its reduced complexity. L2 regularization would help smooth out a model with higher variance, but potentially increase the bias of a more or adequately biased model. The simple cnn did perform better on test, so I think it was adequately biased as is, but the difference in accuracy is vanishingly small.
